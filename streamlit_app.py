...
# Main Header
st.set_page_config(
    page_title="Ecosoul Biz-Hub Analytics Dashboard",
    page_icon="🌴",
    layout="wide",
...
def main_dashboard():
    ...
    # Reading Excel files from GitHub (Excel files must be in .xlsx format)
    inventory_data = pd.read_excel(base_url + "Inventory-Database.xlsx", sheet_name="Inventory_S-D")
    ...
    zoho_db = pd.read_excel(base_url + "Zoho_as_of_Today.xlsx", sheet_name='Pivot_RM_Calculator')
    zoho_pivot = pd.read_excel(base_url + "Zoho_as_of_Today.xlsx", sheet_name='zoho_pivot')
    
    threeg_order = pd.read_excel(base_url + "USA-3G_WH.xlsx", sheet_name='3G-Orders')
    threeg_inventory = pd.read_excel(base_url + "USA-3G_WH.xlsx", sheet_name='3G-Inventory')
    threeg_ageing = pd.read_excel(base_url + "USA-3G_WH.xlsx", sheet_name='3G-Aging')
    ...
    if main_page == "Main":
      ...
      # ---------------------- Cradle to Grave Inventory Value ------------------------
        ...
        with col6:
            ...
            material_options = Stock_value['Material'].dropna().unique()
            product_type_options = Stock_value['Source'].dropna().unique()
            ...
            amazon_total_value = (
                filtered_df['Value_Amazon-USA'].sum() +
                filtered_df['Value_Amazon-Canada'].sum() +
                filtered_df['Value_Amazon-Germany'].sum() +
                filtered_df['Value_Amazon-UK'].sum() +
    ...
    ## --------------------------------------------------------------------------- for amazon shipment-------------------------------------------------------------------------------------
            ...
            """, unsafe_allow_html=True)
            amazon_shipment = amz_shipment.copy()
            # **Display Header with Banner**
    ...
    # __________________________________________________________________________ for Warehouses(3G & Updike) ____________________________________________________________________________________________
        with tab3:  # Warehouses
            threeg_inventory = threeg_inventory.copy()
            ...
            if st.session_state.get('page') == '3G Details':
                ...
                # Remove 'Sample' SKU
                threeg_inventory = threeg_inventory[threeg_inventory['SKU'] != "Sample"]

                # Convert Date column to datetime and get the latest date
                threeg_inventory['Date'] = pd.to_datetime(threeg_inventory['Date'])
                latest_date = threeg_inventory['Date'].max().strftime('%d-%b-%y')

                # Rename columns for clarity
                threeg_inventory = threeg_inventory.rename(columns={
                ...
                total_sellable_box = threeg_inventory['Sellable Stock Value (box)'].sum()
                total_on_hand_box = threeg_inventory['Total 3G Stock Value (box)'].sum()
                total_reserved_box = threeg_inventory['Reserved Stock Value (box)'].sum()
                total_3g_on_hand = threeg_inventory['Total 3G Stock (case)'].sum()
                total_3g_committed = threeg_inventory['Reserved Stock (case)'].sum()
                total_3g_fulfillable = threeg_inventory['Sellable Stock (case)'].sum()
                ...
                if "All" not in material_filter:
                    ...
                    if "All" not in sku_filter:
                        threeg_inventory = threeg_inventory[threeg_inventory['SKU'].isin(sku_filter)]
                    if "All" not in material_filter:
                        threeg_inventory = threeg_inventory[threeg_inventory['Material'].isin(material_filter)]

                # Group by 'Material' and sum values for Total 3G Stock Value (box)
                material_summary = threeg_inventory.groupby('Material').agg({'Sellable Stock Value (box)': 'sum', 'Total 3G Stock Value (box)': 'sum',
                ...
                threeg_inventory = threeg_inventory.loc[:, ['SKU', 'Material', 'Total 3G Stock (case)', 'Sellable Stock (case)', 'Reserved Stock (case)', 
                                                '3G_to_Amazon_CA', '3G_to_Amazon_US', 'Amazon', 'Retail', 'Walmart', 'Samples']]
                st.dataframe(threeg_inventory, width=2500, height=500, use_container_width=True, hide_index=None)    

                # Group by 'Material' and sum the 'Sellable' values
                threeg_inventory = threeg_inventory.groupby('Material')['Sellable Stock (case)'].sum().reset_index()

                # Rename column for clarity
                threeg_inventory = threeg_inventory.rename(columns={"Sellable Stock (case)": "Sellable"})

                # Handle missing values in 'Material' column
                threeg_inventory = threeg_inventory.dropna(subset=['Material'])  # Remove rows where Material is NaN
                threeg_inventory['Material'] = threeg_inventory['Material'].astype(str)  # Convert to string
                total_sellable = threeg_inventory['Sellable'].sum()
        ...
        ## ==================================================================== 3g orders data ========================================================================================
                ...
                threeg_orders = threeg_order.copy()
                # Rename columns for clarity
    ...
    #==================================================================== Updike orders Tracking Report===================================================================================
                ...
                updike_orders=updike_order.copy()
                # Rename columns for clarity
    ...
    if "page" not in st.session_state:
        ...
        retail_overview = us_retail_overview.copy()
        # Define tabs for navigation
    ...
    ## Showing respective data based on the button clicked ======================================== USA Overview =======================================================================
            if st.session_state.get('page') == 'Retail USA Overview':
                ...
                metrics = {
                                "Total PO Served": retail_overview["PO_Number"].nunique(),
                                "PO SKU Qty Asked": retail_overview["SKU_Qty"].sum(),
                                "Invoice Qty Delivered": retail_overview["Invoice_Qty"].sum(),
                                "Total PO Amount": f"${retail_overview['PO_Sales'].sum():,.2f}",
                                "Total Invoice Amount": f"${retail_overview['total_sales'].sum():,.2f}",
                                "Difference Amount": f"${retail_overview['Amt Diff'].sum():,.2f} 🔻 Difference Qty: {retail_overview['Qty Diff'].fillna(0).sum():,.2f}"}
                
                # Display Key Metrics
                st.subheader("🧐 Key Metrics")
                metric_cols = st.columns(len(metrics))
                for col, (title, value) in zip(metric_cols, metrics.items()):
                    col.markdown(f"""
                        <div style="text-align: center; font-weight: bold; background-color:rgb(6, 126, 98); padding: 10px; height: 120px; border-radius: 5px;">
                ...
                df_status = retail_overview.groupby("New_PO_Delivery_Status", as_index=False)["PO_Number"].nunique()
                fig3 = px.bar(df_status, x="New_PO_Delivery_Status", y="PO_Number", color="New_PO_Delivery_Status", title="PO Count by Delivery Status", text_auto=True)
                st.plotly_chart(fig3, use_container_width=True)

            if st.session_state.get('page') == 'SPS':
                st.markdown(""" <div style="margin: 20px 0;"> <div class="banner">SPS Overview</div>
                    </div> <style> .banner {  background-color: white;  /* White background */
                    color: black;  /* Black text */ font-size: 25px;  /* Adjust font size */
                    font-weight: bold; text-align: center;  /* Center align */
                    padding: 8px;  /* Add padding for better spacing */ border-radius: 5px;  /* Rounded corners */
            ...
            if st.session_state.get('page') == 'Kehe':
                st.markdown(""" <div style="margin: 20px 0;"> <div class="banner">Kehe Overview</div>
                    </div> <style> .banner {  background-color: white;  /* White background */
                    color: black;  /* Black text */ font-size: 25px;  /* Adjust font size */
                    font-weight: bold; text-align: center;  /* Center align */
                    padding: 8px;  /* Add padding for better spacing */ border-radius: 5px;  /* Rounded corners */
            ...
            if st.session_state.get('page') == 'Sprouts':
                st.markdown(""" <div style="margin: 20px 0;"> <div class="banner">Sprouts Overview</div>
                    </div> <style> .banner {  background-color: white;  /* White background */
                    color: black;  /* Black text */ font-size: 25px;  /* Adjust font size */
                    font-weight: bold; text-align: center;  /* Center align */
                    padding: 8px;  /* Add padding for better spacing */ border-radius: 5px;  /* Rounded corners */
            ...
            if st.session_state.get('page') == 'Costco':
                st.markdown(""" <div style="margin: 20px 0;"> <div class="banner">Costco Overview</div>
                    </div> <style> .banner {  background-color: white;  /* White background */
                    color: black;  /* Black text */ font-size: 25px;  /* Adjust font size */
                    font-weight: bold; text-align: center;  /* Center align */
                    padding: 8px;  /* Add padding for better spacing */ border-radius: 5px;  /* Rounded corners */
        ...
        with tab2: # India
            st.subheader("Retail India🛒🛍 ")
            if st.session_state.get('page') == 'Retail India':
                st.markdown(""" <div style="margin: 20px 0;"> <div class="banner">Retail India Overview</div>
                    </div> <style> .banner {  background-color: white;  /* White background */
                    color: black;  /* Black text */ font-size: 25px;  /* Adjust font size */
                    font-weight: bold; text-align: center;  /* Center align */
                    padding: 8px;  /* Add padding for better spacing */ border-radius: 5px;  /* Rounded corners */
                    border: 1px solid black;  /* Black border for emphasis */ </style> """, unsafe_allow_html=True)
        with tab3: # Demand & Supply
            st.subheader("Demand & Supply- East & West🛒🛍 ")
            st.markdown(""" <div style="margin: 20px 0;"> <div class="banner">Demand & Supply</div>
                </div> <style> .banner {  background-color: white;  /* White background */
                color: black;  /* Black text */ font-size: 25px;  /* Adjust font size */
                font-weight: bold; text-align: center;  /* Center align */
                padding: 8px;  /* Add padding for better spacing */ border-radius: 5px;  /* Rounded corners */
        ...
        with tab4: # India
            st.subheader("Walmart🛒 ")
            st.markdown(""" <div style="margin: 20px 0;"> <div class="banner">Walmart Demand & Supply</div>
                        </div> <style> .banner {  background-color: white;  /* White background */
                        color: black;  /* Black text */ font-size: 25px;  /* Adjust font size */
                        font-weight: bold; text-align: center;  /* Center align */
                        padding: 8px;  /* Add padding for better spacing */ border-radius: 5px;  /* Rounded corners */
    ...
    # __________________________________________________________________________ for Zoho ____________________________________________________________________________________________
            ...
            zoho_pivot_db = zoho_pivot.copy()
            zoho_dbms =zoho_db.copy()

            zoho_pivot_db = zoho_pivot_db.drop(columns=['Total'], errors='ignore')
            
            # **Key Metrics Calculation**
            total_banglore = zoho_pivot_db["Bangalore"].sum()
            total_muzaf = zoho_pivot_db["Muzaffarnagar"].sum()
            total_noida = zoho_pivot_db["Noida-63"].sum()
            ...
            with col1:
                unique_parents = zoho_pivot_db['Parent'].unique()
            ...
            with col3:
                unique_warehouses = zoho_dbms['Warehouse'].unique()
            ...
            with col4:
                unique_combination_types = zoho_dbms['Combination_Type'].unique()
            ...
            if "All" not in selected_parents:
                zoho_pivot_db = zoho_pivot_db[zoho_pivot_db['Parent'].isin(selected_parents)]
                zoho_dbms = zoho_dbms[zoho_dbms['Parent'].isin(selected_parents)]  # Ensure Parent is filtered in zoho_db as well

            if "All" not in selected_skus:
                # Find related Parent(sku) from zoho_db where SKU matches selected SKU
                related_parents = zoho_dbms[zoho_dbms['SKU'].isin(selected_skus)]['Parent'].unique()

                # Filter both DataFrames based on related Parent(sku)
                zoho_pivot_db = zoho_pivot_db[zoho_pivot_db['Parent'].isin(related_parents)]
                zoho_dbms = zoho_dbms[zoho_dbms['SKU'].isin(selected_skus)]

            if "All" not in selected_warehouses:
                zoho_dbms = zoho_dbms[zoho_dbms['Warehouse'].isin(selected_warehouses)]

            if "All" not in selected_combination_types:
                zoho_dbms = zoho_dbms[zoho_dbms['Combination_Type'].isin(selected_combination_types)]
            ...
            st.markdown("""
                ...
                """, unsafe_allow_html=True)
            st.dataframe(zoho_pivot_db, width=2500, height=500, use_container_width=True, hide_index=None)
            ...
            st.markdown("""
                ...
                """, unsafe_allow_html=True)
            st.dataframe(zoho_dbms, width=2500, height=500, use_container_width=True, hide_index=None)
    ...
    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ USA website email +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            if st.session_state.page == 'USA':
                ...
                website_usa = shopify_customer.copy()
        ...
        # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ India website email +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                ...
                st.markdown("""
                    ...
                    <style> .banner {
                            ...
                            border-radius: 5px;
                            border: 1px solid black; } </style> """, unsafe_allow_html=True)
                website_india = shopify_customer.copy()
    ...
    # 📨 Customer Email Tab +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        with tab2:  
            ...
            cust_email_usa = shopify_customer[shopify_customer['shipping_country'] != 'India']
        ...
        # ____________________________________________________ for India cusotmer data _________________________________________________________
            ...
            cust_email_india = shopify_customer[shopify_customer['shipping_country'] == 'India']
